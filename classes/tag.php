...
class tag{
	...
	public function insert(){
		...
		try {
			...
			$stmt = $conn->prepare("
				insert into tag(name, id_tag_super, image) 
				values (:name, :id_tag_super, :image) ");
		    ...
		    echo 'ERROR - tag.php|insert: ' . $e->getMessage();
		}
	}
	...
	public function get_list_by_products($id_products, $order){
	...
	global $conn;
		...
		try {
			...
			$stmt = $conn->prepare("
				select * 
				from tag
				where id_tag in (select id_tag from product_tag where id_product in ($id_products)) ".$order);
		    ...
		    echo 'ERROR: - tag.php|get_list_by_products' . $e->getMessage();
		}
