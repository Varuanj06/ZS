...
class order_address{
	...
	public function insert(){
		...
		try {
			...
			$stmt = $conn->prepare("
				insert into order_address(id_order_address, id_fb_user, name, mobile_number, address, landmark, city, state, pin_code, email, date_add, date_update) 
				values (:id_order_address, :id_fb_user, :name, :mobile_number, :address, :landmark, :city, :state, :pin_code, :email, now(), now()) ");
		    ...
		    echo 'ERROR - order_address.php|insert: ' . $e->getMessage();
		    return false;
		}
	}
		...
		try {
			...
			$stmt = $conn->prepare("
				...
				set
					...
					email  					= :email,
					date_update 			= now()
				where id_order_address 		= :id_order_address
		    ...
		    echo 'ERROR - order_address.php|update: ' . $e->getMessage();
		}
	}
	...
	public function map($id_order_address, $id_fb_user){
		global $conn;
		...
		try {
		    ...
		    echo 'ERROR: - order_address.php|map' . $e->getMessage();
		}
	}
	...
	public function delete(){
		...
		try {
		    ...
		    echo 'ERROR - order_address.php|delete: ' . $e->getMessage();
		    return false;
		}
	}

	//MAXIMUM
	public function max_id_order_address($id_fb_user){
		global $conn;

		try {

			$stmt = $conn->prepare("
				select COALESCE(MAX(id_order_address)+1,1) AS max 
				from order_address
		    ...
		    echo 'ERROR: - order_address.php|max_id_order_address' . $e->getMessage();
		}
	}
	...
	public function get_list($id_fb_user, $order){
		...
		try {
			...
			foreach ($results as $reg) {
				...
				$order_address->mapea($reg);

				array_push($list, $order_address);
			}

	    } catch(PDOException $e) {
		    echo 'ERROR: - order_address.php|get_list' . $e->getMessage();
		}
