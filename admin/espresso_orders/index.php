...
<body>
	...
	<?php 
		...
		if(isset($_POST['action']) && $_POST['action'] == '1'){//erase
			...
			$espresso_products->map($order_detail->get_id_product());
			...
			$message->set_id_fb_user($id_fb_user);
			$message->set_id_message($message->max_id_message($id_fb_user));
			$message->set_message("Dear Customer , As we are unable to deliver the product - ".$espresso_products->get_name()." - , within the SLA period, the amount is being refunded to your Miracas Account. You can use this to purchase any product of your choosing from the store. We apologize for the inconvenience this may have caused you.");
			$message->set_from('system');
			$message->set_id_message_conversation("-1");
		...
		/* #### ONLY ORDERS THAT DOESN'T HAVE NORMAL DETAILS/PRODUCTS #### */

		$sql_espresso_only 	= " and (select coalesce(count(*),0) from order_detail where id_order=order.id_order and order_type = 'espresso') > 0 ";
		
		/* #### APPLY FILTERS TO THE ORDERS #### */

		$orders = $order->get_list(" $sql_status $sql_dates $sql_email $sql_mobile_number $sql_id_order $sql_espresso_only order by date_done desc "); 
	?>
		...
		<div class="content">
			...
			<form action="" method="post" name="form">
				...
				<?php 
					...
					foreach ($orders as $row){
						...
						foreach ($details as $row_details) {
							$espresso_products	= new espresso_products();
							$espresso_products->map($row_details->get_id_product());

							$espresso_keywords = new espresso_keywords();
							$espresso_keywords->map($espresso_products->get_id_keyword());

							if(!in_array($espresso_products->get_id_keyword(), $all_keywords)){
								$all_keywords[] = $espresso_products->get_id_keyword();
							}
						}
					}
				?>
				...
				<table class="table table-condensed table-bordered">
					...
					<tbody>
						<?php 
							foreach ($all_keywords as $current_id_keyword) {  
								$espresso_keywords = new espresso_keywords();
								$espresso_keywords->map($current_id_keyword);
						?>

							<tr>
								<td colspan="10" class="alert alert-warning text-center">
									<h5><?php echo $espresso_keywords->get_keyword(); ?></h5>
								</td>
							</tr>
							...
							<?php
								foreach ($orders as $row){
									...
									foreach ($details as $row_details) {
										...
										if($espresso_products->get_id_keyword() == $current_id_keyword){
											$keyword_found = true;
										}
									}
												...
												<?php
													foreach ($details as $row) {
														...
														if($row->get_order_type() == 'espresso'){
															...
															$img_link 			= $espresso_products->get_image_link();
															$name 				= $espresso_products->get_name();

															$espresso_keywords = new espresso_keywords();
												        	$espresso_keywords->map($espresso_products->get_id_keyword());

												        	$keyword 			= $espresso_keywords->get_keyword();
														}else{
															$product 			= new product();
															$product->map($row->get_id_product());

															$link 				= $product->get_link();
															$img_link 			= $product->get_image_link();
												        ...
												        <tr>
												        	<td class="item_description">
												        		<a target="_blank" href="<?php echo $link; ?>">
												        			<img style="width:50px !important;" src="<?php echo $img_link; ?>" alt="">
												        		</a>
												        		...
												        		<div>
												        			<p class="badge badge-info"><?php echo $keyword; ?></p>
												        		</div>
	...
	<script>jQuery('a[href="../espresso_orders"]').parents('li').addClass('active');</script>
