	...
	/* ====================================================================== *
 	 ...
 	 * ====================================================================== */			
		...
		if(isset($_REQUEST['ResponseCode'])){
			...
			$hash = "$ebskey|".urlencode($account_id)."|".urlencode($total_ammount)."|".urlencode($current_id_order)."|".$return_url."|".urlencode($mode);
			$secure_hash = md5($hash);

			if(isset($response['ResponseCode']) && isset($response['MerchantRefNo']) && isset($response['Amount']) && isset($response['SecureHash']) && $response['ResponseCode'] == '0' && $response['MerchantRefNo'] == $current_id_order && floatval($response['Amount']) == floatval($total_ammount) ){
				...
				$error = '<div class="alert alert-danger"><strong>Oops</strong>, something went wrong with your payment, please try again.</div>';
			}
		}
        ...
        if(isset($_POST["CHECKSUMHASH"]) && $error == ''){
			...
			if($isValidChecksum == "TRUE") {
				if ($_POST["STATUS"] == "TXN_SUCCESS") {
					...
					// Verify transaction 
					/*
					$url 	= 'https://'.$PAYTM_DOMAIN.'/oltp/HANDLER_INTERNAL/getTxnStatus?JsonData={"MID":"'.PAYTM_MERCHANT_MID.'","ORDERID":"'.$current_id_order.'","CHECKSUMHASH":"'.$order->get_paytm_checksum().'"}';
...
-->
			...
			<img src="payments1.png"/>
				...
				<div class="btn-group" data-toggle="buttons">
				  <?php if($total_ammount<1480 && $total_ammount >480 || $total_ammount ==0 || $found_in_keywords == 'yes'){ ?>
...
-->
				  ...
				  <?php if($id_fb_user=='694133180' || $id_fb_user=='10152767632557633'){ ?>
				  <label class="btn btn-primary <?php if($order->get_payment_method() == 'Pay Online' && $order->get_online_payment() == 'PAYTM'){echo "active";}; ?> ">
				  	<i class="glyphicon glyphicon-ok"></i>
