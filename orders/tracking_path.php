...
<?php
	...
	function make_tracking_path($id_order){
		...
		$staticDate     = $order->get_date_placed();
		$status_admin   = $order->get_status_admin();

		if($status_admin == 'GROUP ORDER CREATED'){
			$html = '';
			$html.= "<div class='alert alert-warning'>Please note that your order will be processed only after your friend has joined the deal and completed his/her purchase</div>";
			$html.= "<div class='alert alert-warning'>tracking not available</div>";
			return $html;
		}
