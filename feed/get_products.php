 ...
 * ====================================================================== */	
	...
	$id_espresso_keyword 	= isset($_GET['id_espresso_keyword'])?$_GET['id_espresso_keyword']:'';
	if($id_espresso_keyword != ''){
		$q = "Espresso keyword";
	}
	...
	if( $q != '' ){
		...
		}else if($id_espresso_keyword != ''){ // only products with id_tag
			$products = $espresso_products->get_search($id_espresso_keyword, $user_gender_slash, $user_age_slash, "");
...
-->		
	...
	<div class="filters_container">
		<ul class="custom-filter" id="sort">
			<li><a href="#" data-sort-by="media-box-id-product" <?php echo $_GET['popular'] != 'yes' ? 'class="selected"' : ''; ?>>Newest First</a></li>
			<li><a href="#" data-sort-by="media-box-view-count" <?php echo $_GET['popular'] == 'yes' ? 'class="selected"' : ''; ?>>Most Popular</a></li>
		</ul>
	...
	<div id="grid"> 
		...
		<?php 
			...
			foreach ($products as $row) {
				...
				$price 					= $id_espresso_keyword=='' ? get_the_price($id_product) : get_the_price_espresso($id_product);
				$discount 				= $id_espresso_keyword=='' ? get_the_discount($id_product, $price) : get_the_discount_espresso($id_product, $price);
				$keyword_discount 		= $id_espresso_keyword=='' ? get_the_keyword_discount($id_product, $price) : 0;
				$colors 				= $attribute->get_colors_of_product($id_product_prestashop);
				$sizes 					= $attribute->get_sizes_of_product($id_product_prestashop);
				$list_stock 			= $id_espresso_keyword=='' ? get_product_stock($id_product) : array();
				...
				// IF ITS A LUCKY SIZE PRODUCT (AND IT IS NOT AN ESPRESSO PRODUCT) AND THE TOTAL STOCK IS 0 THEN IS NOT VALID
				else{
		...
		?>				
	        <div 
	        	class="media-box <?php if(count($list_stock)>0){echo 'lucky_sizes';}else{echo 'normal_products';} ?> <?php echo "bg_$count_bg"; ?>" 
	        	style="display:none;" 
	        	
	        >
				<a href="product_details.php?<?php echo $id_espresso_keyword=='' ? 'id_product' : 'id_espresso_product' ?>=<?php echo $row->get_id_product(); ?><?php echo str_replace("?", "&", $extra_param); ?>" target="blank">
		            <div class="media-box-image">
		                <div data-thumbnail="<?php echo $img_url; ?>" data-width="600" data-height="600"></div>
		                
		                <?php if($id_espresso_keyword==''){ ?>
			                <span class="count">
				...
				<a href="product_details.php?<?php echo $id_espresso_keyword=='' ? 'id_product' : 'id_espresso_product' ?>=<?php echo $row->get_id_product(); ?><?php echo str_replace("?", "&", $extra_param); ?>" target="blank">
		            <div 
		            	class="media-box-content" 
		            	data-idproductprestashop="<?php echo $id_product_prestashop; ?>" 
		            	data-idproduct="<?php echo $row->get_id_product(); ?>" 
		            ...
		            >
		                <div class="media-box-text" style="margin: 0 !important;">
		                	...
		                	<div class="pull-right" style="margin-left: 10px;">
	                			<?php if($id_espresso_keyword=='' && $active_session === true){ ?>
	                				<?php 
	                					$fb_user_product_save->set_id_fb_user($id_fb_user);
	                					$fb_user_product_save->set_id_product($id_product);
	                				?>
		                			<div class="media-box-wishlist" 
		                				data-id-product="<?php echo $id_product; ?>" 
		                				data-saved="<i class='fa fa-check'></i>&nbsp; Added" 
		                				data-notsaved="<i class='fa fa-bookmark'></i>&nbsp; Add to Whishlist"
		                			> 
				                		<?php 
				                			if ($fb_user_product_save->exists()){ 
				                				echo "<i class='fa fa-check'></i>&nbsp; Added"; 
				                			}else{ 
				                				echo "<i class='fa fa-bookmark'></i>&nbsp; Add to Whishlist"; 
				                			} 
				                		?>
				                	</div>
				                <?php } ?>
	                		</div>	
	                		<div class="media-box-name">
							...
							<?php if($keyword_discount<=0){ ?>
			                	...
			                	<div>
			                		<span class="media-box-price">
			                			<?php if($discount>0){ ?>
			                				<span class="media-box-price-discount">₹<?php echo number_format($price, 2); ?></span>
			                			<?php } ?>
							...
							<div <?php if(count($sizes)==0){ ?> style="opacity: 0;" <?php } ?>>
	            				Size:
	            				<?php 
		                			foreach ($sizes as $size){ 
		                				...
		                				if(count($list_stock) && !in_array($size, $sizes_with_stock)){continue;}
	                		...
	                		<div <?php if(count($colors)==0){ ?> style="opacity: 0;"  <?php } ?>>
		                		<span style="color: #669A67;">Available in <?php echo count($colors) ?> colors &nbsp;&nbsp;</span>
