 ...
 * ====================================================================== */	
	...
	// IF A KEYWORD IS SENT, AND WITHOUT THE SET_GENDER PROPERTY, THEN GET THE FIRST GENDER FROM THE KEYWORD

	if(isset($_GET['q']) && !isset($_GET['set_gender'])){
		$keyword->map($_GET['q']);

		$genders = explode("/", $keyword->get_genders());
 ...
 * ====================================================================== */	
	...
	if ($q == '' && strpos($_SERVER['HTTP_HOST'], 'miracas.in') !== false) {
		//echo "<script>location.href='global.php';</script>";
		echo "<script>location.href='global.php?set_gender=$user_gender';</script>";
		exit();
	}
?>
...
<!--<?php require_once("../mobile_app.php"); ?>-->
	...
	/* ====================================================================== *
 	...
 	-->
			...
			<?php if( $q == '' ){ ?>
				...
				<?php //$url = isset($_GET['new_homepage']) ? 'get_keywords_new.php' : 'get_keywords.php'; ?>
				<?php $url = 'get_keywords.php'; ?>

				<script>
					$('#keywords').html('<img src="../includes/img/loader.gif" class="loading" />');
					$.get('<?php echo $url; ?>?set_gender=<?php echo $user_gender; ?>', function(rs){
 	...
 	-->			
			...
			<?php }else if( $q != ''){ ?>
				...
				<?php 
					...
					if($id_tag != ''){ // only products with id_tag
				...
				<?php $show_collections = isset($_GET['collections']) ? $_GET['collections'] : '';  ?>

				<?php 
					if(
						( isset($_GET['q']) || isset($_GET['id_tag']) ) && 			// if its a search by a keyword or by id_tag
						!isset($_GET['date']) && 									// if it doesn't have a specific date specified
						( count($products) >= 100 || $show_collections == 'yes' ) 	// if it has more than 100 products or forced to show the collections
					){ 
				?>
					...
					<script>
						...
						$.get('get_collections.php?set_gender=<?php echo $user_gender; ?>&q=<?php echo $q; ?>&id_tag=<?php echo $id_tag; ?>', function(rs){
				...
				?>
					...
					<script>
						<?php $popular = isset($_GET['popular']) ? $_GET['popular'] : ''; ?> 
						<?php if (strpos($_SERVER['HTTP_HOST'], 'miracas.in') !== false) { ?>
							var url = "get_products_global.php";
						<?php }else{ ?>
							var url = "get_products.php";
						<?php } ?>

						$('#products').html('<img src="../includes/img/loader.gif" class="loading" />');
						$.get(url+'?set_gender=<?php echo $user_gender; ?>&q=<?php echo $q; ?>&size=<?php echo $lucky_size; ?>&id_tag=<?php echo $id_tag; ?>&usr=<?php echo $q_fb_user; ?>&date=<?php echo $date; ?>&popular=<?php echo $popular; ?>&id_espresso_keyword=<?php echo $id_espresso_keyword; ?>', function(rs){
	...
	/* ====================================================================== *
 	...
 	-->	
		...
		<script>
	 	 	...
	 	 	$('body').on('click', '.media-box-wishlist', function(e){
	 	 		e.preventDefault();
...
<script type="text/javascript">
  (function() {
    ...
    pa.src = ('https:' == document.location.protocol ? 'https:' : 'http:') + "//tag.marinsm.com/serve/5b98d20981371e9b85000023.js";
